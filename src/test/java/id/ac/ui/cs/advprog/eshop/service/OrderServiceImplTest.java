...
class OrderServiceImplTest {
    ...
    void setUp() {
        ...
        Order order2 = new Order("7f9e15bb-4b15-42f4-aebc-c3af385fb078",
                products, 17085700001L, "Safira Sudrajat");
        orders.add(order2);
    }
    ...
    void testUpdateStatusInvalidOrderId() {
        doReturn(null).when(orderRepository).findById("zccc");

        assertThrows(NoSuchElementException.class,
                () -> orderService.updateStatus("zccc", OrderStatus.SUCCESS.getValue()));
    ...
    void testFindByIdIfNotFound() {
        doReturn(null).when(orderRepository).findById("zccc");
        assertNull(orderService.findById("zccc"));
    }

    @Test
    void testFindAllByAuthorAuthorCorrect() {
        Order order = orders.get(1);
        List<Order> orderList = new ArrayList<>();
        orderList.add(order);
        orderList.add(orders.get(0));
    ...
    @Test
    void testFindAllByAuthorAllLowercase() {
        Order order = orders.get(1);
